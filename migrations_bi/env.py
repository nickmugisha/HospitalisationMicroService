...
from services.bi.config import DATABASE_URL
from services.bi import models  # noqa
config=context.config
if config.config_file_name is not None: fileConfig(config.config_file_name)
target_metadata=BIBase.metadata
def offline():
...
def online():
    ...
    with connectable.connect() as connection:
        connection.exec_driver_sql("SET time_zone = '+00:00'"); context.configure(connection=connection,target_metadata=target_metadata,compare_type=True)
        with context.begin_transaction(): context.run_migrations()
