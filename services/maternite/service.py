...
from services.maternite.config import ACCUEIL_GRPC_TARGET,BILLING_GRPC_TARGET,MATERNITE_CURRENCY_CODE,MATERNITE_DELIVERY_CHARGE_MINOR,SERVICE_VERSION
...
class MaterniteService(maternite_pb2_grpc.MaterniteServiceServicer):
    ...
    def RegisterNewborn(self,request,context):
        ...
        try:
            ...
            session.add(n); session.commit(); session.refresh(n); return maternite_pb2.NewbornResponse(newborn=newborn_proto(n),replayed=False)
        finally: session.close()
